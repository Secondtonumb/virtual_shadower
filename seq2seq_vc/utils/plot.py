...
def plot_attention(array, figname, figsize=(6, 4), dpi=500, origin="upper"):
    shape = array.shape
...
def plot_max_attention(array, figname, figsize=(6, 4), dpi=500, origin="upper"):
    import numpy as np
    focus_max_index = np.argmax(array.max(axis=-1).mean(axis=-1))
    focus_max_index = np.unravel_index(focus_max_index, array.shape[:2])
    
    array_max = array[focus_max_index[0], focus_max_index[1]]
    plt.figure(figsize=figsize, dpi=dpi)
    plt.imshow(array_max, aspect="auto")
    plt.xlabel("Input")
    plt.ylabel("Output")
